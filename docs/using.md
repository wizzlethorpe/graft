...
- **Importing grafts.** **Import grafts** on the Settings tab builds pasted grafts into the world: one entry, a list of entries, or a whole `grafts.json`, typed in or loaded from a file. Documents file under the folders the entries name. Pre-build transforms run with `"world"` as the module id. Nothing is tracked afterwards: nothing is pruned, and an existing world document with the same id is only overwritten if an import wrote it.
