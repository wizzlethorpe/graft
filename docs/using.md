...
- **Building a file.** **Import grafts** on the Settings tab builds a `grafts.json` file into the world. Documents are filed under a folder with the name you give, one per sidebar. Pre-build transforms run with `"world"` as the module id. Nothing is tracked afterwards: nothing is pruned, and an existing world document with the same id is only overwritten if graft built it.
